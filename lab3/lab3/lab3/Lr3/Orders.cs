...
namespace Lr3
{
    public partial class Orders : Form
    {
        ...
        private async void button4_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                SqlCommand command = new SqlCommand("SELECT Order_chip, Order_date, Client, Phone FROM [Orders] WHERE Date=@date", sqlConnection);
                ...
                while (await reader.ReadAsync())
                {
                    listBox3.Items.Add(Convert.ToString(reader["Order_chip"]) + "\t" + 
                        (Convert.ToString(reader["Order_date"])) + "\t" + 
                        (Convert.ToString(reader["Client"])) + "\t" + 
                        (Convert.ToString(reader["Phone"])));
